...
// Some agents (notably Sonnet variants) occasionally over-escape whitespace
// control characters in JSON tool-call arguments — writing `\n` as the
// two-character sequence backslash+n instead of the single newline byte.
// Discord renders those literally. Normalize at the boundary so every agent
// benefits. Only handles the common cases; leaves already-correct strings
// untouched.
function unescapeAgentContent(s) {
...
export const handlers = {
  async discord_send_message(args) {
    ...
    if (typeof args.content === 'string' && args.content.length > 2000) {
      return `Error: content is ${args.content.length} chars; Discord max is 2000. Split into multiple messages or use an embed description (max 4096).`;
    }
    if (args.embeds !== undefined) {
      if (!Array.isArray(args.embeds)) return 'Error: embeds must be an array.';
      if (args.embeds.length > 10) return `Error: max 10 embeds per message; got ${args.embeds.length}.`;
    }
    if (args.reply_to_message_id !== undefined) {
      requireSnowflake(args.reply_to_message_id, 'reply_to_message_id');
    }
    const body = {};
    if (args.content !== undefined) body.content = unescapeAgentContent(args.content);
  ...
  async discord_list_messages(args) {
    requireSnowflake(args.channel_id, 'channel_id');
    // Discord rejects multiple cursors; enforce at most one.
    const cursors = ['before', 'after', 'around'].filter((k) => args[k] != null);
    if (cursors.length > 1) {
      return `Error: only one of before/after/around may be set; got ${cursors.join(', ')}.`;
    }
    for (const c of cursors) requireSnowflake(args[c], c);
